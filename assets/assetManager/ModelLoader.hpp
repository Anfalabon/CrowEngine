...
template<typename T> std::vector<T> ModelLoader::GetBufferFromFile(std::string &bufferContentString)
{
    ...
    //Checking this beacuse of a weired bug that i had before...
    if (bufferContentString.size() <= 0)
    {
        return std::vector<T>{};
    }
    else if (bufferContentString[bufferContentString.size() - 1] != '*')
    {
        bufferContentString.push_back(',');
        bufferContentString.push_back('*');
    }

    //bool typeIsFloat = std::is_same<T, float>::value;
